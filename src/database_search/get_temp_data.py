...
def get_cambridge_bay_temperature():
    start_total = time.time()
    
    location_code = 'CBYIP'
    # location_code = 'CBYSS.M1'
    ...
    try:
        ...
        ctd_devices = [d for d in devices if d.get('deviceCategoryCode') == 'CTD']
        
        if not ctd_devices:
            total_time = time.time() - start_total
            print(f"Total time: {total_time:.2f}s")
            return "No CTD devices found"
        
        print(f"Found {len(ctd_devices)} CTD devices")
        
        # try each device until we find one with data
        for ctd_device in ctd_devices:
            device_code = ctd_device['deviceCode']
            device_name = ctd_device['deviceName']
            ...
            # look for temperature sensor data
            ...
            for sensor_info in sensor_data_list:
                if isinstance(sensor_info, dict):
                    ...
                    if property_code == 'seawatertemperature':
                        temp_sensor_data = sensor_info
                        print(f"Found seawater temperature sensor: {sensor_name}")
                        break
            ...
            if temp_sensor_data:
                ...
                if values:
                    ...
                    latest_temp = values[-1]
                    latest_time = sample_times[-1] if sample_times else 'unknown time'
                    
                    print(f"Got {len(values)} temperature reading(s)")
                    step_3_start = time.time()
                    result = f"Ocean temperature in Cambridge Bay: {latest_temp}{unit} (from {device_name} at {latest_time})"
                    total_time = time.time() - start_total
                    ...
                    print(f"No temperature values found from {device_name}, trying next device...")
                    continue
        
        # if we get here, no devices had temperature data
        total_time = time.time() - start_total
        print(f"Total time: {total_time:.2f}s")
        return "No temperature data found from any CTD devices in Cambridge Bay"
...
if __name__ == "__main__":
    ...
    result = get_cambridge_bay_temperature()
    print(f"\nResult: {result}")